...
public class CollectionRepository implements _BaseRepository<Collection>, _Logger<String> {
        ...
        try (var conn = new OracleDbConfiguration().getConnection();
            ...
            while (rsCartas.next()) {
                try{var stmt = conn.prepareStatement("UPDATE " + CardRepository.TB_NAME + " SET COD_COLECAO = ? WHERE COD_CARTAS = ?");
                    ...
                    stmt.executeUpdate();
                    logWarn("Carta adicionada a coleção com sucesso");
                }
    ...
    public void adicionarColecaoNaCarta(Card carta, Collection colecao){
        try (var conn = new OracleDbConfiguration().getConnection();
             var stmtCartas = conn.prepareStatement("SELECT * FROM " + CardRepository.TB_NAME + " WHERE NOME = ?")) {
            stmtCartas.setString(1, carta.getNome());
            var rsCartas = stmtCartas.executeQuery();
            while (rsCartas.next()) {
                try{var stmt = conn.prepareStatement("UPDATE " + CardRepository.TB_NAME + " SET COD_COLECAO = ? WHERE COD_CARTAS = ?");
